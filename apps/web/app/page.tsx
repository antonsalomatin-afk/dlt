...
type View = 'setup' | 'practice' | 'history' | 'mistakes' | 'favorites' | 'progress' | 'exam' | 'exam-history' | 'concepts';
...
export default function Home() {
  ...
  return <main className="mx-auto flex min-h-svh max-w-5xl flex-col px-5 py-7 sm:px-10 sm:py-10">
    ...
    <div className="onboarding-grid">
      ...
      {view === 'concepts' && session ? <Concepts key={`${session.token}:concepts`} session={session} onBack={() => setView(feedReturn)} onExpired={() => { setView('setup'); setSession(null); setChoice(null); setError(true); setMessage('Your session has ended. Close and reopen ThaiDLT in Telegram, then try again.'); }} /> : view === 'exam-history' && session ? <ExamHistory key={`${session.token}:exam-history`} session={session} onBack={() => setView(feedReturn)} onExpired={() => { setView('setup'); setSession(null); setChoice(null); setError(true); setMessage('Your session has ended. Close and reopen ThaiDLT in Telegram, then try again.'); }} /> : view === 'exam' && session ? <Exam key={`${session.token}:exam`} session={session} onBack={() => setView(feedReturn)} onHistory={() => setView('exam-history')} onVehicle={() => { setSession({ ...session, user: { ...session.user, selectedVehicleType: null } }); setView('setup'); setChoice(null); setMessage('Choose and save your vehicle to continue.'); }} onExpired={() => { setView('setup'); setSession(null); setChoice(null); setError(true); setMessage('Your session has ended. Close and reopen ThaiDLT in Telegram, then try again.'); }} /> : view === 'progress' && session ? <Progress key={session.token} session={session} onBack={() => setView(feedReturn)} onExpired={() => { setView('setup'); setSession(null); setChoice(null); setError(true); setMessage('Your session has ended. Close and reopen ThaiDLT in Telegram, then try again.'); }} /> : view === 'favorites' && session ? <Favorites key={session.token} session={session} onBack={() => setView(feedReturn)} onExpired={() => { setView('setup'); setSession(null); setChoice(null); setError(true); setMessage('Your session has ended. Close and reopen ThaiDLT in Telegram, then try again.'); }} /> : (view === 'history' || view === 'mistakes') && session ? <History key={`${session.token}:${view}`} kind={view} session={session} onBack={() => setView(feedReturn)} onExpired={() => { setView('setup'); setSession(null); setChoice(null); setError(true); setMessage('Your session has ended. Close and reopen ThaiDLT in Telegram, then try again.'); }} /> : view === 'practice' && session ? <Practice key={`${session.token}:${session.user.selectedVehicleType}`} session={session} onVehicle={(missing) => { if (missing) setSession({ ...session, user: { ...session.user, selectedVehicleType: null } }); setView('setup'); setChoice(null); setMessage('Choose and save your vehicle to continue.'); }} onProgress={() => { setFeedReturn('practice'); setView('progress'); }} onHistory={() => { setFeedReturn('practice'); setView('history'); }} onMistakes={() => { setFeedReturn('practice'); setView('mistakes'); }} onFavorites={() => { setFeedReturn('practice'); setView('favorites'); }} onExam={() => { setFeedReturn('practice'); setView('exam'); }} onExamHistory={() => { setFeedReturn('practice'); setView('exam-history'); }} onConcepts={() => { setFeedReturn('practice'); setView('concepts'); }} onExpired={() => { setView('setup'); setSession(null); setChoice(null); setError(true); setMessage('Your session has ended. Close and reopen ThaiDLT in Telegram, then try again.'); }} /> : <section className="panel" aria-labelledby="onboarding-title" aria-busy={pending}>
        ...
        {session && <div className="panel-nav setup-links"><button className="secondary" disabled={pending} onClick={() => { setFeedReturn('setup'); setView('progress'); }}>Progress</button><button className="secondary" disabled={pending} onClick={() => { setFeedReturn('setup'); setView('history'); }}>History</button><button className="secondary" disabled={pending} onClick={() => { setFeedReturn('setup'); setView('mistakes'); }}>Mistakes</button><button className="secondary" disabled={pending} onClick={() => { setFeedReturn('setup'); setView('favorites'); }}>Favorites</button><button className="secondary" disabled={pending} onClick={() => { setFeedReturn('setup'); setView('exam-history'); }}>Exams</button><button className="secondary" disabled={pending} onClick={() => { setFeedReturn('setup'); setView('concepts'); }}>Concepts</button></div>}
        {session?.user.selectedVehicleType && choice === session.user.selectedVehicleType && <div className="panel-nav setup-links"><button className="primary next" disabled={pending} onClick={() => setView('practice')}>Start practice</button><button className="primary next exam-entry" disabled={pending} onClick={() => { setFeedReturn('setup'); setView('exam'); }}>Mock exam</button></div>}
