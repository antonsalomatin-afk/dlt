...
export function Practice({ session, onExpired, onVehicle }: { session: Session; onExpired: () => void; onVehicle: (missing?: boolean) => void }) {
  ...
  return <section className="panel practice" aria-labelledby="practice-title" aria-busy={pending}>
    ...
    <button className="secondary" onClick={() => { active.current = false; onVehicle(); }}>Change vehicle</button>
    {presentation && <>
