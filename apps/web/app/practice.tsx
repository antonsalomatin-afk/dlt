...
export function Practice({ session, onExpired, onVehicle, onHistory, onMistakes, onFavorites }: { session: Session; onExpired: () => void; onVehicle: (missing?: boolean) => void; onHistory: () => void; onMistakes: () => void; onFavorites: () => void }) {
  ...
  return <section className="panel practice" aria-labelledby="practice-title" aria-busy={categoriesPending || pending || favoritePending}>
    ...
    <div className="panel-nav"><button className="secondary" onClick={() => { active.current = false; onVehicle(); }}>Change vehicle</button><button className="secondary" onClick={() => { active.current = false; onHistory(); }}>History</button><button className="secondary" onClick={() => { active.current = false; onMistakes(); }}>Mistakes</button><button className="secondary" onClick={() => { active.current = false; onFavorites(); }}>Favorites</button></div>
