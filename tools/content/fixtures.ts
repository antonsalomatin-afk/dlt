...
const fixtureSchema = z.strictObject({
  ...
  questions: z.array(z.strictObject({
    id: z.string().regex(/^[a-z][a-z0-9-]{1,63}$/), category: z.enum(categoryKeys),
...
export async function importFixtures(database: PrismaClient, input: unknown) {
  ...
  await database.$transaction(async (transaction) => {
    ...
    for (const question of fixtures.questions) {
      const { id: stableKey, category, choices, ...wording } = question;
      ...
      const data = { ...wording, categoryId: fixtureId(`category:${category}`), sourceType: 'FIXTURE' as const,
  ...
  return { questions: 25, choices: 100, categories: 5 };
}
